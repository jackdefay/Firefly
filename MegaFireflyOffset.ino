...
#define FREQUENCY 1000 //Use this to generalize program to upload to more than one arduino
#define OFFSET 100
...
int mod = 0;  //declares the modifier value as a global variable
	...
	unsigned long predictedTime1 = 0;  //declares "frequency," which keeps track of the frequencies that each of the connected fireflies
 	unsigned long predictedTime2 = 0;  //are blinking at, calculated based on the values stored in the "input" arrays
    unsigned long predictedTime3 = 0;

    unsigned long avgPredictedTime = 0;
...
void loop()
{
	...
	while(button)  //starts main portion of the program when the button is pressed
	{
	 ...
	 /* if(millis()%2000==0){  //print statements for troubleshooting
	    ...
	    {
		 	predictedTime1 = input1[iteration1-1][0] + FREQUENCY;
		    started = true;
	    }

	    if(iteration2 > 3)
	    {
		 	predictedTime2 = input2[iteration2-1][0] + FREQUENCY;
		    started = true;
	    }

	  if(iteration3 > 3)
	  {
	  		predictedTime3 = input3[iteration3-1][0] + FREQUENCY;
            started = true;
	  }
	  
	  if((started))  //calculates the average to evaluate how synchronized this firefly is
	  {
	  	avgPredictedTime = ((predictedTime1+predictedTime2+predictedTime3+(previousMillis+FREQUENCY))/4);  //calculates the arithmatic mean rounded off to a whole number(in milliseconds) - also, averages all connected fireflies AND itself to avoid differences in data.
	  }

	  if((started) && ((previousMillis+FREQUENCY) < (avgPredictedTime))) mod++;  //shifts the frequency of the firefly longer by a tenth of a second if the average is larger

	  else if((started) && ((previousMillis+FREQUENCY) > (avgPredictedTime))) mod--;  //shifts the frequency of the firefly down if the average is shorter

	  else if((started) && (((previousMillis+FREQUENCY) == predictedTime1) && (previousMillis+FREQUENCY) == predictedTime2 && (previousMillis+FREQUENCY) == predictedTime3))
	  { 
